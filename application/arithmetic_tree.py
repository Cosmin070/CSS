...
def compute_operation(operands, operations):
    if not all(len(x) < 128 for x in operands):
        raise large_number_exception("Number too large.")
    if operations[-1] != '√':
        val2 = operands.pop()
        val1 = operands.pop()
        op = operations.pop()
        if op in '*/' and ((len(val1) + len(val2) >= 8 and abs(len(val1) - len(val2)) < 3)
            ...
            raise large_number_exception("Number too large for multiplication/division.")
        if op == "^" and (len(val2) > 10 or (len(val1) + len(val2) >= 10)):
            raise large_number_exception("Number too large for power.")
...
def evaluate(expression):
    ...
    while i < len(expression):
        if expression[i] == ' ':
            ...
            if check_close_parenthesis(operations):
                raise invalid_expression_exception('The expression is INVALID. You can not close a parenthesis without to open one.')
            ...
            try:
                if check_next_char(expression, i):
                    raise invalid_expression_exception('The expression is INVALID. You can not have successions of operators.')
            except Exception as e:
                raise invalid_expression_exception("The expression is INVALID. Operation without both or any operand.")
    ...
    while len(operations) != 0:
        if check_parenthesis(operations):
            raise invalid_expression_exception('The expression is INVALID. Check parenthesis.')
        compute_operation(operands, operations)
    return operands[-1]
